...
""" Define the root URL for downloading the dataset and set up paths
for storing it """
...
def fetch_housing_data(
    ...
    This function downloads the housing dataset from the given URL,
    and extracts it into the specified directory (housing_path). If
    the directory doesn't exist, it creates the directory.
...
def load_housing_data(housing_path=HOUSING_PATH):
    ...
    This function reads a CSV file containing housing data
    located at the specified path and returns it as a pandas DataFrame.
    The default path is set to the global variable `HOUSING_PATH`.

    Args:
    housing_path (str): The directory path where the 'housing.csv'
    file is located. Defaults to the value of `HOUSING_PATH`.

    Returns:
    pd.DataFrame: A pandas DataFrame containing the housing data
...
def income_cat_proportions(data):
    ...
    This function computes the relative proportions of each unique income
    category found in the 'income_cat' column of the provided DataFrame.
    The proportions are calculated as the count of each category divided
    by the total number of entries in the DataFrame, providing insight
    into the distribution of income
    categories.

    Args:
    data (pd.DataFrame): A pandas DataFrame that must contain an
    'income_cat' column,which holds the income category values.

    Returns:
    pd.Series: A pandas Series containing the proportions of each
